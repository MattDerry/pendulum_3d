...
for i in range(max_trial_num):
    filestr = '/home/mderry/cps_data/user_' + str(user_number) + '/user_' + str(user_number) + '_trial_' + str(i) + '_combined_trajectory.mat'
    (Xd, Ud) = dsys.load_state_trajectory(filestr)
...
mp.hold(True)
mp.plot(raw_trust_log, color='purple', linewidth=1)
...
mp.legend()
mp.title('Alternative Trust Formulations (aggressive vs. conservative)')
